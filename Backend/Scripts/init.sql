...
CREATE TABLE users (
                       ...
                       password_hash VARCHAR(255) NOT NULL, -- Adjust length based on hashing algorithm (e.g., 60 for bcrypt)
...
CREATE INDEX idx_users_email ON users(email); -- For lookup/recovery

-- POSTS table
CREATE INDEX idx_posts_user_id ON posts(user_id);
CREATE INDEX idx_posts_created_at ON posts(created_at DESC); -- For feeds
...
CREATE INDEX idx_likes_content ON likes(content_type, content_id); -- For fetching likes for a post/comment
...
CREATE INDEX idx_messages_user_id ON messages(user_id); -- For finding messages by a user
